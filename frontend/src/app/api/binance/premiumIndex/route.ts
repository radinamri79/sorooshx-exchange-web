...
import { generateMockPremiumIndex } from '../lib/mock-data';
...
export async function GET(request: NextRequest) {
  ...
  if (USE_MOCK_DATA || isBinanceBlocked()) {
    return NextResponse.json(generateMockPremiumIndex(symbol));
  }
  
  const urls = [
    `${BINANCE_REST_URL}/fapi/v1/premiumIndex?symbol=${symbol.toUpperCase()}`,
    `${BINANCE_TESTNET_URL}/fapi/v1/premiumIndex?symbol=${symbol.toUpperCase()}`,
  ];
